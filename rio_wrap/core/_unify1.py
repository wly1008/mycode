...
def _unify(raster_in, dst_in=None, out_path=None,
         ...
         csize=False, size=None, tolerance=0,
         # unify=0, 
         # unify_options=None,
         delete=False,
         stacklevel=3,
    ...
    with ExitStack() as stack:
        ...
        if crop:
            ...
            if dst_in:
                ...
                # arr_crop = dst.read_masks(out_shape=arr.shape)
                arr_crop = dst.read_masks(out_shape=(dst.count, win_right.width, win_right.height))
            ...
            if arr_crop.ndim == 2:
                arr_crop = np.array([arr_crop])
            
            if not ((arr_crop.shape[0] == 1) or (arr_crop.shape[0] == arr.shape[0])):
                raise ValueError('arr_crop 与 arr波段数不匹配且arr_crop不为单波段, 无法广播')
